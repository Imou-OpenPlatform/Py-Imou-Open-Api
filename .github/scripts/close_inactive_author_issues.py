...
DEFAULT_REPO_NAME = "Py-Imou-Open-Api"
...
def _repo_root() -> Path:
    ...
    for p in (here.parent.parent, *here.parent.parents):
        if (p / "pyproject.toml").is_file() and (p / ".github").is_dir():
            return p
...
def _exclude_labels() -> frozenset[str]:
    raw = (
        os.environ.get("INACTIVE_EXCLUDE_LABELS") or "no-auto-close,enhancement"
    ).strip()
    return frozenset(x.strip().lower() for x in raw.split(",") if x.strip())
...
def list_issue_comments(owner: str, repo: str, number: int) -> list[dict]:
    ...
    while True:
        ...
        url = (
            f"https://api.github.com/repos/{owner}/{repo}/issues/{number}/comments?{q}"
        )
        batch = gh("GET", url)
        if not isinstance(batch, list) or not batch:
            break
